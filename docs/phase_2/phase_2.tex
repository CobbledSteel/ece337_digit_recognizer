...
\pagestyle{fancy}
\lhead{ECE337 Project Phase 2 Report}
...
\\[0.5\baselineskip]
{\large\sc 2/24/2018} % Editor list
...
\texttt{2'b00} & New image data will be sent over MOSI \\ 
\texttt{2'b01} & A new expected label will be sent over MOSI \\ 
\texttt{2'b10} & The weights/biases have changed, flush registers \\ \hline
\end{tabular}
...
\begin{sidewaysfigure}[H]
  ...
  \begin{subfigure}[h]{\textwidth}
    \centering
    \includegraphics[height=2.8cm]{FlushRegistersWave.pdf}
  ...
  \begin{subfigure}[h]{\textwidth}
    \centering
    \includegraphics[height=2.8cm]{WriteImageWave.pdf}
  ...
  \begin{subfigure}[h]{\textwidth}
    \centering
    \includegraphics[height=2.8cm]{WriteLabelWave.pdf}
...
\newpage
Flushing the registers is not necessarily something that will be used that often, but is necessary to include in the design. Flushing the registers is necessary to do when the weights or biases have been overwritten in the flash memory by the controller that is communicating with the digit recognition circuit. To flush the registers, a command code of \texttt{2'b10} has to be transmitted over SPI as the first byte of a packet. Since data in the system is transmitted LSB first, the last two bits are the only pieces of information that matter within the first byte. All data from the controller using the digit recognition system must be written to the MOSI line, the SS should be low on the device, and the SCK should only run for 8 cycles to ensure that only 8 bits of information have been sent. The MISO line is not used in this scenario. \\

To write an image for an operation to be performed on into the circuit, first the command code \texttt{2'b00} must be written through the SPI MOSI. immediately after the command code the first byte of the image is written, followed by the second, followed by the third etc. until all 196 bytes have been written. Then the user must wait a defined amount of time before the result byte can be read out from the SPI MISO. the SS is low whenever the SCK is running. \\

To write an expected label for the current image on into the circuit, first the command code \texttt{2'b01} must be written through the SPI MOSI. immediately after the command code the label value in binary is input. Then the user must wait a defined amount of time before the cost byte can be read out from the SPI MISO. the SS is low whenever the SCK is running.\\
